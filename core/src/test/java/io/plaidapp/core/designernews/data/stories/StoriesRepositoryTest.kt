...
class StoriesRepositoryTest {
    ...
    private val dataSource = Mockito.mock(StoriesRemoteDataSource::class.java)
    ...
    fun loadTopStories_withSuccess() = runBlocking {
        ...
        Mockito.`when`(dataSource.loadTopStories(1)).thenReturn(result)
    ...
    fun loadTopStories_withError() = runBlocking {
        ...
        Mockito.`when`(dataSource.loadTopStories(2)).thenReturn(result)
        var sourceLoadingFailed = false
        // Given a callback where we check the validity of the data received
        val callback = object : LoadSourceCallback {
            override fun sourceLoaded(data: List<PlaidItem>?, page: Int, source: String) {
                Assert.fail("Load shouldn't have succeeded")
            }
    ...
    fun search_withSuccess() = runBlocking {
        ...
        val result = Result.Success(stories)
        Mockito.`when`(dataSource.search(query, 1)).thenReturn(result)
        var sourceLoaded = false
        // Given a callback where we check the validity of the data received
        val callback = object : LoadSourceCallback {
            override fun sourceLoaded(data: List<PlaidItem>?, page: Int, source: String) {
        ...
        // When loading the recent stories
    ...
    fun search_withError() = runBlocking {
        ...
        val result = Result.Error(IOException("error"))
        Mockito.`when`(dataSource.search(query, 2)).thenReturn(result)
        var sourceLoadingFailed = false
        // Given a callback where we check the validity of the data received
        val callback = object : LoadSourceCallback {
            override fun sourceLoaded(data: List<PlaidItem>?, page: Int, source: String) {
